...
## Corpus Check
- 12 files · ~8,248 words
- Verdict: corpus is large enough that graph structure adds value.

## Summary
- 92 nodes · 119 edges · 12 communities detected
- Extraction: 75% EXTRACTED · 25% INFERRED · 0% AMBIGUOUS · INFERRED: 30 edges (avg confidence: 0.8)
...
3. `detect_candlestick_patterns()` - 9 edges
4. `calculate_rsi()` - 7 edges
5. `calculate_indian_transaction_costs()` - 6 edges
6. `init_db()` - 6 edges
7. `generate_options_signal()` - 6 edges
8. `TestIndicators` - 5 edges
9. `TestPatterns` - 5 edges
10. `calculate_daily_pivots()` - 5 edges
...
- `generate_swing_blueprint()` --calls--> `calculate_rsi()`  [INFERRED]
...
Cohesion: 0.17
Nodes (14): Simulates trades chronologically on a historical dataframe to verify profitabili, Simulates trades chronologically on a historical dataframe to verify profitabili, run_backtest_simulation(), calculate_atr(), calculate_bollinger_bands(), calculate_ichimoku(), calculate_macd(), calculate_moving_averages() (+6 more)

### Community 1 - "Community 1"
Cohesion: 0.17
Nodes (8): TestIndicators, calculate_daily_pivots(), calculate_rsi(), Calculates standard Floor Pivot Points based on previous session's OHLC., Calculates Relative Strength Index (RSI) using Wilder's smoothing technique., generate_options_signal(), Directional options strategy recommendations (Calls or Puts) based on index mome, Directional options strategy recommendations (Calls or Puts) based on index mome
...
- **Why does `run_backtest_simulation()` connect `Community 0` to `Community 1`, `Community 4`, `Community 5`?**
  _High betweenness centrality (0.135) - this node is a cross-community bridge._
- **Why does `calculate_rsi()` connect `Community 1` to `Community 0`, `Community 7`?**
  _High betweenness centrality (0.107) - this node is a cross-community bridge._
- **Why does `detect_candlestick_patterns()` connect `Community 4` to `Community 0`, `Community 1`?**
